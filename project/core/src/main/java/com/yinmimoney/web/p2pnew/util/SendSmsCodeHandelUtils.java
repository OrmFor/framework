...
public class SendSmsCodeHandelUtils {
	...
	public boolean sendMobileCode(String mobilePhone, EnumSendSmsCodeType enumSendSmsCodeType, String ip) {
		if (Strings.isNullOrEmpty(mobilePhone)) {
			throw new BussinessException(StatusCode.ERROR_MOBILE_EMPTY);
		}
		if (enumSendSmsCodeType == null) {
			throw new BussinessException(StatusCode.ERROR_MOBILE_CODE_TYPE);
		}
		if (!RegexUtil.checkInput("^1[0-9]{10}$", Strings.nullToEmpty(mobilePhone))) {
			throw new BussinessException(StatusCode.ERROR_MOBILE_FORMAT);
		}
		...
		case TYPE_RESET_PASSWORD:
			...
			if (admin == null) {
				throw new BussinessException(StatusCode.ERROR_MOBILE_NOT_EXISTS);
			}
			if (admin.getStatus().intValue() != EnumAdminStatus.STATUS_NORMAL.getStatus()) {
				throw new BussinessException(StatusCode.ERROR_ACCOUNT_LOCK_OR_LEAVE);
			}
			// 短信内容填充数据
		...
		case TYPE_QUICK_LOGIN:
			...
			break;
		default:
			throw new BussinessException(StatusCode.ERROR_PARAM);
		}
		return true;
	}
}
