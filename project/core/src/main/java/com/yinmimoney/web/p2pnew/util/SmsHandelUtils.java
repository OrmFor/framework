...
import com.yinmimoney.web.p2pnew.enums.EnumMessageChannel;
import com.yinmimoney.web.p2pnew.enums.EnumNoticeNid;
import com.yinmimoney.web.p2pnew.enums.EnumNoticeSend;
...
public class SmsHandelUtils {
	...
	public boolean checkMobileCodeIsEqual(String mobilePhone, String code) {
		...
		if (!Strings.isNullOrEmpty(code_error)) {
			...
			if (errorNumber >= 5) {
				...
				throw new BussinessException(StatusCode.ERROR_MOBILE_CODE_EXPIRED);
			}
		}
		...
		if (!code_.equalsIgnoreCase(code)) {
			...
			throw new BussinessException(StatusCode.ERROR_MOBILE_CODE);
		}
		return true;
	}
	...
	public boolean send(EnumNoticeNid enumNoticeNid, EnumNoticeType enumNoticeType, String sentUser, String receiveUser, String receiveAddr, HashMap<String, Object> sendData,
		...
		if (enumNoticeNid == null) {
			throw new BussinessException(StatusCode.ERROR_NOTIC_NID_EMPTY);
		}
		if (enumNoticeType == null) {
			throw new BussinessException(StatusCode.ERROR_NOTIC_TYPE_EMPTY);
		}
		if (Strings.isNullOrEmpty(sentUser)) {
			sentUser = "1";
		}
		if (Strings.isNullOrEmpty(receiveUser)) {
			throw new BussinessException(StatusCode.ERROR_NOTIC_RECEIVE_USER_EMPTY);
		}
		if (receiveAddr == null) {
			throw new BussinessException(StatusCode.ERROR_NOTIC_RECEIVE_ADDRESS_EMPTY);
		}
		if (validate == null)
			validate = false;
		// 查找通知类型配置
	...
	private String validateSendMobileCodeLimit(String key) {
		...
		if (!Strings.isNullOrEmpty(value)) {
			if (Integer.parseInt(value) > 4) {
				throw new BussinessException(StatusCode.ERROR_MOBILE_CODE_SEND_LIMIT);
			}
		}
		return value;
	}
